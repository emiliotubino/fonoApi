...
import PatientEvaluation, { EvaluationStatus } from '../models/patientEvaluation';
...
router.get('/:patientId/patient-evaluations', authMiddleware, requireRole(['superadmin']), async (req: AuthRequest, res: Response) => {
  try {
    ...
    const { status, templateId, categoryId, startDate, endDate } = req.query;
    ...
    if (categoryId) {
      query['templateSnapshot.categories'] = categoryId;
    }
